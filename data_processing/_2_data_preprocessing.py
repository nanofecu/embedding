...
class MaterialsTextProcessor:
    ...
    def get_ordered_integer_formula(el_amt, max_denominator=1000):
        ...
        # Check for division by zero or nearly zero (e.g., very small floating point values).
        if abs(g) < 1e-10:
            print(f"Warning: Division by zero or near-zero detected for {el_amt}. Skipping...")
            return ""
        
        try:
            d = {k: round(v / g) for k, v in el_amt.items()}
        except OverflowError:
            print(f"OverflowError encountered for {el_amt} with gcd {g}. Skipping...")
            return ""
...
# Initialize an instance of the MaterialsTextProcessor class
processor = MaterialsTextProcessor()


# The above code is referenced from the work by:
...
# ======================== The code below is written by the author self ========================
...
def process_text_1(text):
    """
    Process the given text using the MaterialsTextProcessor, 
    Normalizer, and SentSplitter. 
    ...
    """
    # Process the text
    processed_tokens, material_list = processor.process(text)
    filtered_text = ' '.join(processed_tokens)
    paragraph_processed_1 = normalizer.normalize(filtered_text)
    paragraph_processed_2 = splitter.split(paragraph_processed_1)
    return paragraph_processed_2


def process_and_save(paragraph, directory_path, filename):
    """
    Process the paragraph and save the processed sentences to a file.
    
    :param paragraph: Paragraph to be processed.
    :type paragraph: List[str]
    :param directory_path: Directory path where the file will be saved.
    :type directory_path: str
    :param filename: Name of the file where processed sentences will be saved.
    :type filename: str
    """
    filename = os.path.join(directory_path, filename)
    new_sentences = []
    # Process sentences. If a sentence contains '.', split it.
    for sentence in paragraph:
        splitted = sentence.split('. ')
        i = 0
        while i < len(splitted):
            # If the next sentence has less than 40 characters, merge with the current sentence.
            merge_count = 0
            while i < len(splitted) - 1 and len(splitted[i]) <= 40 and merge_count < 10:
                splitted[i] += '. ' + splitted[i + 1]
                splitted[i + 1] = ''
                i += 1
                merge_count += 1
            i += 1
        new_sentences.extend(s for s in splitted if s)  # Add only non-empty sentences

    # Save processed sentences to a temporary file.
    temp_filename = os.path.join(directory_path, "temp_" + filename)
    ...
    # Iterate through the entire file, check each line of text, if the current line has less than 50 characters, delete the line.
    with open(temp_filename, 'r') as f:
        lines = f.readlines()

    with open(filename, 'w') as f:
        for line in lines:
            # Filter out lines containing "https://" and "http://"
...
def read_txt_files_from_directory(directory_path):
    """
    Read all txt files from the specified directory, process them and save the processed content.
    
    :param directory_path: The path of the directory containing txt files.
    :type directory_path: str
    """
    ...
    for txt_file in tqdm(txt_files, desc="Reading files", ncols=100, unit="file"):

        file_path = os.path.join(directory_path, txt_file)
        
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
            txt_file = txt_file.replace(".txt", "_processed.txt")
            paragraph_processed_2 = process_text_1(content)
            process_and_save(paragraph_processed_2, directory_path, txt_file)


# Call the function to read txt file content from the "directory_path" directory
